...
\begin{document}
	...
	Key-value (neboli Klíč-hodnota) databázové systémy~\cite{amaz-key-value-db, ytb-nosql-db}, dále jen KDBS, jsou jedním z typů NoSQL DBS~\cite{nosql}. Databáze je navržena pro uživatelsky přátelský přístup k datům pomocí triviálních příkazů. Základem manipulace s daty je přístup k objektům v databázi prostřednictvím klíčů. V KDBS se hodnota ukládá k danému klíči, přičemž klíč může být například "jmeno\_uzivatele\_03", a hodnota může být řetězec "Jan Novák". Při práci s komplexnějšími daty mohou být hodnoty reprezentovány prostřednictvím JSON objektů~\cite{json}. Klíč "detail\_uzivatele\_03"~může pak jako hodnotu ukládat například JSON objekt vybraného uživatele popisující jeho jméno, věk, adresu nebo kontakt (viz JSON objekt~\ref{json-hodnota}).
	...
	\begin{lstlisting}[language=json,firstnumber=1]
{
  "jmeno": "Jan Novak",
   ...
   "kontakt": [
	...
	\end{lstlisting}
	\caption{Příklad JSON objektu uloženého jako hodnotu pro klíč "detail\_uzivatele\_03"}
	\label{json-hodnota}
	\end{figure}	
